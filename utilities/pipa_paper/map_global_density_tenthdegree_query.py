...
query = {  'query': '''SELECT
  FLOOR(avg_lat*10)/10 lat_bin,
  IF((max_lon > 170
      AND min_lon < -170), IF(RAND()>.5,-177+3*FLOOR(POW(RAND(),2)*10)/10,177-3*FLOOR(POW(RAND(),2)*10)/10), FLOOR(avg_lon*10)/10) lon_bin,
  COUNT(*) number
FROM
  [scratch_david_gapanalysis.ave_locations_2012_2015]
WHERE
  YEAR(TIMESTAMP(date)) = 2015
...
# plt.show()
plt.savefig("vessel_density_2015_cleaned.png",bbox_inches='tight',dpi=450,transparent=True,pad_inches=0)
